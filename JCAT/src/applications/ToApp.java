...
public class ToApp extends FswApp {
	...
	public void defineCmds() {
		...
		cmdDataBuf[13] = 0x30;
		cmdDataBuf[14] = 0x31;
		cmdPkt = new CmdPkt(PREFIX_STR, "Ena Tlm", CMD_MID, CMD_FC_ENA_TLM,
				cmdDataBuf, cmdDataLen);
		commands.set(CMD_FC_ENA_TLM, cmdPkt);

		/*
		 * The following doesn't work & not sure why cmdPkt = new
		 * CmdPkt(PREFIX_STR, "Ena Tlm", CMD_MID, CMD_FC_ENA_TLM, 15); // One IP
		 * address parameter cmdPkt.addParam(new
		 * CmdStrParam("IP Address","127.000.000.001", 15));
		 * CmdList.set(CMD_FC_ENA_TLM,cmdPkt);
		 */
		cmdPkt = new CmdPkt(PREFIX_STR, "Add Pkt", CMD_MID, CMD_FC_ADD_PKT, 7);
		cmdPkt.addParam(new CmdIntParam("Message ID",
				new ParameterDetails(true), "2048", 2)); // // 3840 = 0xF00
															// (ExApp), 2048 =
															// 0x800 (ES HK)
		cmdPkt.addParam(new CmdIntParam("Pkt Size", new ParameterDetails(true),
				"50", 2));
		cmdPkt.addParam(new CmdIntParam("SB QoS", new ParameterDetails(true),
				"0", 2));
		cmdPkt.addParam(new CmdIntParam("Buffer Cnt",
				new ParameterDetails(true), "1", 1));
		cmdPkt.loadParamList();
		commands.set(CMD_FC_ADD_PKT, cmdPkt);

		cmdPkt = new CmdPkt(PREFIX_STR, "Rem Pkt", CMD_MID, CMD_FC_REM_PKT, 2); // One
																				// 2
																				// byte
																				// parameter
		cmdPkt.addParam(new CmdIntParam("Message ID",
				new ParameterDetails(true), "3840", 2));
		cmdPkt.loadParamList();
	...
	public CmdPkt getEnaTlmCmd() {
		...
		cmdDataBuf[13] = 0x30;
		cmdDataBuf[14] = 0x31;
		return new CmdPkt(PREFIX_STR, "Ena Tlm", CMD_MID, CMD_FC_ENA_TLM,
				cmdDataBuf, cmdDataLen);
	}
