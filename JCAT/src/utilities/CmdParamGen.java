...
public class CmdParamGen {
	private enum DataType {
		uint8("uint8", 1), uint16("uint16", 2), uint32("uint32", 4), int8(
				"int8", 1), int16("int16", 2), int32("int32", 4), Char("char",
				1);

		private static final DataType[] dataTypes = new DataType[] {
				DataType.uint8, DataType.uint16, DataType.uint32,
				DataType.int8, DataType.int16, DataType.int32, DataType.Char };
	...
	private enum PrimitiveType {
		...
		private static final PrimitiveType[] primitiveTypes = new PrimitiveType[] {
		...
		private CmdParam getCmdParam(String name,
				ParameterDetails parameterDetails, String defValue,
				int totalBytes) {
			if (hasName("str"))
				return new CmdStrParam(name, parameterDetails, defValue,
						totalBytes);
			else if (hasName("int"))
				return new CmdIntParam(name, parameterDetails, defValue,
						totalBytes);
			else if (hasName("tbd"))
				return new CmdStrParam(name, parameterDetails, defValue,
						totalBytes);
	...
	public static final CmdParam getCmdParam(String name, String type,
			String primitive, String bytes, ParameterDetails parameterDetails,
			String defValue) {
		System.out.println("NAME: " + name + ", TYPE: " + type
				+ ", PRIMITIVE: " + primitive + ", BYTES: " + bytes + ".");

		for (PrimitiveType primitiveType : PrimitiveType.primitiveTypes)
			if (primitiveType.hasName(primitive))
				return primitiveType.getCmdParam(name, parameterDetails,
